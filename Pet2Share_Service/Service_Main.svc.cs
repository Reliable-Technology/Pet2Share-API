    ...
    public class Service_Main : IService_Main
    {
        ...
        public GeneralUpdateResponse InsertPetProfile(PetProfileInsertRequest PetObj)
        {
            ...
            try
            {
                ...
                if (Result.IsSuccessful)
                {
                    PetProfileResultResp = new GeneralUpdateResponse { Total = 1, Results = (new Pet2Share_API.Utility.BoolExt[] { new BoolExt(true, "Pet added successfully!", Result.UpdatedId) }), ErrorMsg = null };
                }
                else
                {
